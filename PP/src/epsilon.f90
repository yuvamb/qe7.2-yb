...
!
!
!!! ----------------------- CUSTOM MODULE BY YUVAM B ----------------------
MODULE custom_addition
  USE kinds,        ONLY : DP
...
CONTAINS
  ...
  SUBROUTINE convert_and_save_wavefunction(psi, psi_r, psi_r_3d, dfftp, nbands, nx, ny, nz, ngrid, ik)
    !----------------------------------------------------------------------
    !! Convert the wavefunction at Gamma point from reciprocal to real space 
    !! for all bands and write the output to "wavefunction.dat" in a 3D grid format.
    !!
    !! Arguments:
    !!   psi      : Complex wavefunction in reciprocal space [npw, nbands].
    !!   psi_r    : 1D temporary array for the inverse FFT result.
    !!   psi_r_3d : 3D array to store the real-space wavefunction by band [nbands, nx, ny, nz].
    !!   dfftp    : FFT descriptor (its wave_clock_label will be set to "Wave" if uninitialized).
    !!   nbands   : Number of bands.
    !!   nx,ny,nz : Grid dimensions.
    !!   ngrid    : Total number of grid points (nx * ny * nz).
    !!   ik       : Index of the k-point (usually 1 for Gamma).
    !----------------------------------------------------------------------
    IMPLICIT NONE
    INTEGER, INTENT(IN)       :: nbands, nx, ny, nz, ik, ngrid
    COMPLEX(DP), INTENT(IN)   :: psi(:,:)       ! (npw, nbands)
    TYPE(fft_type_descriptor), INTENT(INOUT) :: dfftp

    COMPLEX(DP), INTENT(INOUT)  :: psi_r(:)       ! 1D temporary array (invFFT result)
    COMPLEX(DP), INTENT(INOUT)  :: psi_r_3d(:,:,:,:)
    INTEGER                     :: iband, ix, iy, iz, unit_out_1, unit_out_2, unit_out_3, i, npw
    INTEGER, DIMENSION(3)       :: hs
    !INTEGER, ALLOCATABLE       :: igk_mapping(:)
    ...
    hs(1) = 1      ! group_size = 1 (no grouping)
    hs(2) = npw    ! true number of plane waves
    hs(3) = 1      ! howmany for the inverse FFT call (usually 1)

    ! Write the input wavefunction in G-space to file.
    filepath = TRIM(tmp_dir)//'/wavefunction_Gspace.dat'
    OPEN(NEWUNIT=unit_out_1, FILE=TRIM(filepath), STATUS='REPLACE', FORM='FORMATTED')
    WRITE(unit_out_1,*) "Wavefunction psi dimensions: ", SIZE(psi,1), " x ", SIZE(psi,2)
    WRITE(unit_out_1,*) "FFT grid dimensions: nx =", nx, " ny =", ny, " nz =", nz
    DO i = 1, SIZE(psi,2)
      WRITE(unit_out_1,*) "Band ", i, ":"
      DO ix = 1, SIZE(psi,1)
          WRITE(unit_out_1, '(F15.9,2X,F15.9)') REAL(psi(ix,i)), AIMAG(psi(ix,i))
      ENDDO
      WRITE(unit_out_1,*) "-------------------------------------"
    END DO
    CLOSE(unit_out_1)

    filepath = TRIM(tmp_dir)//'/wavefunctions_Rspace.dat'
    OPEN(NEWUNIT=unit_out_2, FILE=TRIM(filepath), STATUS='REPLACE', FORM='FORMATTED')
    WRITE(unit_out_2, *) "Wavefunction (real space) at Gamma point:"
    WRITE(unit_out_2, *) "Grid dimensions: nx =", nx, " ny =", ny, " nz =", nz, "(Total points:", ngrid, ")"
    WRITE(unit_out_2, *) "Number of bands:", nbands
    WRITE(unit_out_2, *) "-----------------------------------------------------------"
  
    ! Open a file for the linear (1D) real-space wavefunction output.
    filepath = TRIM(tmp_dir)//'/wavefunctions_Rspace_linear.dat'
    OPEN(NEWUNIT=unit_out_3, FILE=TRIM(filepath), STATUS='REPLACE', FORM='FORMATTED')
    WRITE(unit_out_3, *) "Linear psi_r output"
    WRITE(unit_out_3, *) "Expected dimensions: nx =", nx, " ny =", ny, " nz =", nz, " Total points =", ngrid
  
    DO iband = 1, nbands
         ! Initialize FFT descriptor if its clock label is uninitialized.
         IF ( TRIM(dfftp%wave_clock_label) == '' ) THEN
            dfftp%wave_clock_label = "Wave"
         END IF
         
         ! Perform the inverse FFT for one band.
         CALL wave_g2r( psi(1:npw,iband:iband), psi_r, dfftp, igk_k(1:npw,ik), hs )
             !tgwave_g2r( f_in, f_out, dfft, n, igk )
         
         ! Normalize the wavefunction in real space.
         !psi_r = psi_r / SQRT( REAL(ngrid, DP) )
         !psi_r = psi_r / SQRT( SUM(psi_r(:)) )
         !psi_r = psi_r / SQRT( SUM( ABS(psi_r(:))**2 ) )
         ! Reshape the 1D output into a 3D array and store by band.
         psi_r_3d(iband,:,:,:) = RESHAPE(psi_r, (/ nx, ny, nz /))
  
         WRITE(unit_out_2, *) "Band ", iband, ":"
         DO iz = 1, nz
            WRITE(unit_out_2, *) "Plane z =", iz
            DO iy = 1, ny
               DO ix = 1, nx
                  WRITE(unit_out_2, '(F15.9,2X,F15.9)', ADVANCE="NO") &
                       REAL(psi_r_3d(iband,ix,iy,iz)), AIMAG(psi_r_3d(iband,ix,iy,iz))
               ENDDO
               WRITE(unit_out_2, *)  ! New line after each y row.
            ENDDO
            WRITE(unit_out_2, *)  ! Blank line between z-planes.
         ENDDO
         WRITE(unit_out_2, *) "-----------------------------------------------------------"
  
         ! -------------------------------
         ! Write out the linear fft result for this band.
         ! -------------------------------
         WRITE(unit_out_3, *) "Band ", iband, " linear psi_r values:"
         DO i = 1, SIZE(psi_r)
            WRITE(unit_out_3, '(F15.9,2X,F15.9)') REAL(psi_r(i)), AIMAG(psi_r(i))
         END DO
         WRITE(unit_out_3,*) "-------------------------------------"
    END DO
  ...
  END SUBROUTINE convert_and_save_wavefunction

  SUBROUTINE write_dipole_aux(dipole_, nbnd, fname)
    !----------------------------------------------------------------------
    !! Write the dipole matrix elements to file.
    !!
    !! Arguments:
    !!   dipole_ : Real dipole matrix [3, nbnd, nbnd].
    ...
    REAL(DP), INTENT(IN)      :: dipole_(3, nbnd, nbnd)
    INTEGER                   :: i, j, comp, unit_o
    CHARACTER(256)            :: filepath
    
  
    filepath = TRIM(tmp_dir)//'/'//TRIM(fname)
    OPEN(NEWUNIT=unit_o, FILE=TRIM(filepath), STATUS='REPLACE', FORM='FORMATTED')
    WRITE(unit_o, *) "Dipole matrix (dimensions 3 x", nbnd, "x", nbnd, "):"
    DO i = 1, nbnd
      DO j = 1, nbnd
        WRITE(unit_o, '(A,I3,A,I3,A)') "dipole(:,", i, ",", j, "):"
        DO comp = 1, 3
          WRITE(unit_o, '(F15.9)') dipole_(comp,i,j)
        END DO
        WRITE(unit_o, *)  ! Blank line for separation.
      END DO
    END DO
    CLOSE(unit_o)
  END SUBROUTINE write_dipole_aux


  SUBROUTINE compute_dipole_integrals( psi, psi_r, psi_array, dfftp, n_bands, dipole_matrix, nx, ny, nz, ngrid, ik )
    !----------------------------------------------------------------------
    !! Dipole matrix elements ⟨ψₙ|r|ψₚ⟩ using the real-space wavefunctions obtained by FFT conversion.
    !! 
    !! For a non-orthogonal unit cell the position operator is defined by
    !! mapping fractional coordinates (xi1,xi2,xi3) via the lattice matrix A:
    !!
    !!    r = A * [xi1, xi2, xi3]^T,  with A(:,j) = at(:,j)*alat.
    !!
    !! Fractional coordinates are defined as:
    !!    xi1 = (ix-1)/REAL(nx,DP),  xi2 = (iy-1)/REAL(ny,DP),
    !!    xi3 = (iz-1)/REAL(nz,DP)
    !!
    !! The volume element is dV = det(A)/(nx*ny*nz).
    !!
    !! Arguments:
    !!   psi           : Complex wavefunction [npw, n_bands]
    !!   psi_r         : 1D temporary array for the inverse FFT result.
    !!   psi_array     : 3D array to store the real-space wavefunction by band [n_bands, nx, ny, nz]
    !!   nx, ny, nz    : Grid dimensions.
    !!   ik            : Index of the k-point (usually 1 for Gamma).
    !!   ngrid         : Total number of grid points (nx * ny * nz).
    !!   dfftp         : FFT descriptor (provides grid dims, etc.)
    !!   n_bands       : Number of bands.
    !!   dipole_matrix : Output dipole matrix [3, n_bands, n_bands]
    !----------------------------------------------------------------------
    IMPLICIT NONE

    INTEGER, INTENT(IN) :: n_bands, nx, ny, nz, ik, ngrid
    COMPLEX(DP), INTENT(IN) :: psi(:, :)   ! dimensions: (npw, n_bands)
    TYPE(fft_type_descriptor), INTENT(INOUT) :: dfftp
    !REAL(DP), INTENT(INOUT) :: dipole_tot_matrix(:,:,:)  ! dimensions: (3, n_bands, n_bands)

    INTEGER :: ierr
    INTEGER :: i_band, j_band, ix, iy, iz
    REAL(DP) :: dV, x, y, z, factor
    REAL(DP) :: xfrac, yfrac, zfrac
    REAL(DP), DIMENSION(3,3) :: A
    REAL(DP) :: detA
    REAL(dp), PARAMETER  :: pi = 3.14159_dp

    COMPLEX(DP), INTENT(INOUT) :: psi_array(:,:,:,:)  ! dimensions: (n_bands, nx, ny, nz)
    COMPLEX(DP), INTENT(INOUT) :: psi_r(:)  ! dimensions: (n_bands, nx, ny, nz)
    COMPLEX(DP), INTENT(INOUT) :: dipole_matrix(:,:,:)  ! dimensions: (3, n_bands, n_bands)
    ...
    !----------------------------------------------------------------------
    ! Build the lattice matrix A = at * alat.
    !----------------------------------------------------------------------
    A = 0.0_DP
    DO ix = 1, 3
      DO iy = 1, 3
          A(ix,iy) = at(ix,iy) * alat
      ENDDO
    ENDDO

    !----------------------------------------------------------------------
    ! Unit cell volume determinant.
    !----------------------------------------------------------------------
    ...
    !----------------------------------------------------------------------
    ! Initialize the dipole matrix and psi arrays.
    !----------------------------------------------------------------------

    dipole_matrix(:,:,:) = 0.0_DP
    psi_array(:,:,:,:)= 0.0_DP
    psi_r(:)= 0.0_DP

    CALL convert_and_save_wavefunction(psi, psi_r, psi_array, dfftp, n_bands, nx, ny, nz, ngrid, ik)

    !----------------------------------------------------------------------
    ! Define the volume element: dV = det(A)/(nx*ny*nz)
    !----------------------------------------------------------------------
    dV = detA / REAL(nx*ny*nz, DP)


    !----------------------------------------------------------------------
    ! Loop over the uniform fractional grid, map to Cartesian coords,
    ! and accumulate the dipole integrals.
    !----------------------------------------------------------------------
    DO i_band = 1, n_bands
      DO j_band = 1, n_bands
          DO iz = 1, nz
            zfrac = REAL(iz-1, DP) / REAL(nz, DP)
            DO iy = 1, ny
                yfrac = REAL(iy-1, DP) / REAL(ny, DP)
                DO ix = 1, nx
                  xfrac = REAL(ix-1, DP) / REAL(nx, DP)
                  x = A(1,1)*xfrac + A(1,2)*yfrac + A(1,3)*zfrac
                  y = A(2,1)*xfrac + A(2,2)*yfrac + A(2,3)*zfrac
                  z = A(3,1)*xfrac + A(3,2)*yfrac + A(3,3)*zfrac
                  dipole_matrix(1, i_band, j_band) = dipole_matrix(1, i_band, j_band) + &
                         CONJG( psi_array(i_band, ix, iy, iz) ) * &
                        (( A(1,1) / (2.0_dp * pi * i_c)) * ( exp( 2.0_dp * pi * i_c * xfrac ) - 1.0_dp ) + & 
                         ( A(1,2) / (2.0_dp * pi * i_c)) * ( exp( 2.0_dp * pi * i_c * yfrac ) - 1.0_dp ) + &
                         ( A(1,3) / (2.0_dp * pi * i_c)) * ( exp( 2.0_dp * pi * i_c * zfrac ) - 1.0_dp )) * &
                         psi_array(j_band, ix, iy, iz) * dV
                  dipole_matrix(2, i_band, j_band) = dipole_matrix(2, i_band, j_band) + &
                         CONJG( psi_array(i_band, ix, iy, iz) ) * &
                        (( A(2,1) / (2.0_dp * pi * i_c)) * ( exp( 2.0_dp * pi * i_c * xfrac ) - 1.0_dp ) + & 
                         ( A(2,2) / (2.0_dp * pi * i_c)) * ( exp( 2.0_dp * pi * i_c * yfrac ) - 1.0_dp ) + &
                         ( A(2,3) / (2.0_dp * pi * i_c)) * ( exp( 2.0_dp * pi * i_c * zfrac ) - 1.0_dp )) * &
                         psi_array(j_band, ix, iy, iz) * dV
                  dipole_matrix(3, i_band, j_band) = dipole_matrix(3, i_band, j_band) + &
                         CONJG( psi_array(i_band, ix, iy, iz) ) * &
                        (( A(3,1) / (2.0_dp * pi * i_c)) * ( exp( 2.0_dp * pi * i_c * xfrac ) - 1.0_dp ) + & 
                         ( A(3,2) / (2.0_dp * pi * i_c)) * ( exp( 2.0_dp * pi * i_c * yfrac ) - 1.0_dp ) + &
                         ( A(3,3) / (2.0_dp * pi * i_c)) * ( exp( 2.0_dp * pi * i_c * zfrac ) - 1.0_dp )) * &
                         psi_array(j_band, ix, iy, iz) * dV
                END DO
            END DO
          END DO
  ...
  END SUBROUTINE compute_dipole_integrals

END MODULE custom_addition
!
!!! ----------------------- END OF CUSTOM MODULE BY YUVAM B ----------------------
!
!
!------------------------------
 MODULE grid_module
...
SUBROUTINE eps_calc ( intersmear,intrasmear, nbndmin, nbndmax, shift, metalcalc , nspin)
  ...
  USE klist,                ONLY : nks, nkstot, degauss, ngauss, igk_k, nelec
  ...
  USE custom_addition,      ONLY : compute_dipole_integrals, write_dipole_aux
  ...
  ! local variables
  !
  INTEGER       :: i, ik, iband1, iband2,is, nx, ny, nz, ngrid
  INTEGER       :: iw, iwp, ierr
  ...
  REAL(DP),    ALLOCATABLE :: dipole_orig(:,:,:)
  REAL(DP)                 :: factor
  COMPLEX(DP), ALLOCATABLE :: dipole_aux(:,:,:)
  COMPLEX(DP), ALLOCATABLE :: dipole_matrix(:,:,:)
  COMPLEX(DP), ALLOCATABLE :: psi_array(:,:,:,:)  ! dimensions: (n_bands, nx, ny, nz)
  COMPLEX(DP), ALLOCATABLE :: psi_r(:)  ! dimensions: (ngrid)
  !
...
!
    ...
    !
    ALLOCATE( dipole_orig(3, nbnd, nbnd), STAT=ierr )
    IF (ierr/=0) CALL errore('epsilon','allocating dipole_orig', abs(ierr) )
    !
    ALLOCATE( dipole_aux(3, nbnd, nbnd), STAT=ierr )
    IF (ierr/=0) CALL errore('epsilon','allocating dipole_aux', abs(ierr) )
    !
    ALLOCATE( dipole_matrix(3, nbnd, nbnd), STAT=ierr )
    IF (ierr/=0) CALL errore('epsilon','allocating dipole_matrix', abs(ierr) )
    !
    ALLOCATE( epsr( 3, nw), epsi( 3, nw), eels( 3, nw), ieps(3,nw ), STAT=ierr )
    IF (ierr/=0) CALL errore('epsilon','allocating eps', abs(ierr))
    !
    ALLOCATE( psi_array(nbnd, nx, ny, nz), STAT=ierr )
    IF (ierr /= 0) CALL errore('compute_dipole_integrals','allocating psi_array', ierr)
    !
    ALLOCATE( psi_r( ngrid ), STAT=ierr )
    IF (ierr /= 0) CALL errore('compute_dipole_integrals','allocating psi_r', ierr)
    !
    ...
    !
    IF ( nspin == 1 .OR. nspin == 4) factor = 1.0_DP / ( 2.0_DP * (REAL(nelec,DP)**2) * (REAL(omega,DP)**1.5) * REAL(nkstot,DP) )
    IF ( nspin == 2)                 factor = 1.0_DP / ( (REAL(nelec,DP)**2) * (REAL(omega,DP)**1.5) * REAL(nkstot,DP) )
    !
    ! main kpt loop
    !
    kpt_loop: &
    DO ik = 1, nks
        ...
        CALL compute_dipole_integrals( evc, psi_r, psi_array, dfftp, nbnd, dipole_matrix, nx, ny, nz, ngrid, ik )
        !
        dipole_orig(:,:,:)= tpiba2 * REAL( dipole_aux(:,:,:) * conjg(dipole_aux(:,:,:)), DP )
        dipole(:,:,:)= factor * tpiba2 * REAL( dipole_matrix(:,:,:) * conjg(dipole_matrix(:,:,:)), DP )
                ...
                DO iband1 = nbndmin,nbndmax
                    ...
                    IF ( focc(iband1,ik) >= 0.5d-4*full_occ ) THEN
                        ...
                        DO iw = 1, nw
                            ...
                            w = wgrid(iw)
                            !
                            epsi(:,iw) = epsi(:,iw) + dipole(:,iband1,iband2) * etrans * intersmear * w* &
                                                    RYTOEV**3 * (focc(iband1,ik))/  &
                                                    (( (etrans**2 -w**2 )**2 + intersmear**2 * w**2 ) )

                            epsr(:,iw) = epsr(:,iw) + dipole(:,iband1,iband2) * etrans * RYTOEV**3 * &
                                                    (focc(iband1,ik)) * &
                                                    (etrans**2 - w**2 ) / &
                                                    (( (etrans**2 -w**2 )**2 + intersmear**2 * w**2 ) )
                        ENDDO
        ...
        IF (metalcalc) THEN
            DO iband1 = nbndmin,nbndmax
                ...
                DO iw = 1, nw
                    ...
                    w = wgrid(iw)
                    !
                    epsi(:,iw) = epsi(:,iw) +  dipole(:,iband1,iband1) * etrans**2 * intrasmear * w * &
                                 RYTOEV**2 * w0gauss((et(iband1,ik)-efermi)/degauss, ngauss) / &
                                 (( w**4 + intrasmear**2 * w**2 )*degauss ) * (0.5d0 * full_occ)
                    epsr(:,iw) = epsr(:,iw) - dipole(:,iband1,iband1) * etrans**2 * RYTOEV**2 * &
    ...
    !
    ! Addition of static contribution to the dielectric function
    !
    epsr(:,:) = 1.0_DP + epsr(:,:)
    !
    ...
    IF ( ionode ) THEN
        ...
        !
        CALL eps_writetofile("epsr_custom",desc(1),nw,wgrid,3,epsr,desc2)
        CALL eps_writetofile("epsi_custom",desc(2),nw,wgrid,3,epsi)
        CALL eps_writetofile("eels_custom",desc(3),nw,wgrid,3,eels)
        CALL eps_writetofile("ieps_custom",desc(4),nw,wgrid,3,ieps)
        !
    ENDIF

    CALL write_dipole_aux(dipole_orig, nbnd, 'dipole_matrix.dat')
    CALL write_dipole_aux(dipole, nbnd, 'dipole_matrix_custom.dat')
    DEALLOCATE ( epsr, epsi, eels, ieps)
    !
    ! local cleaning
    !
    DEALLOCATE (  dipole, dipole_aux, dipole_matrix, dipole_orig, psi_array, psi_r )
...
SUBROUTINE dipole_calc( ik, dipole_aux, metalcalc, nbndmin, nbndmax )
  ...
  USE lsda_mod,             ONLY : nspin
  USE fft_base,             ONLY : dfftp
  USE custom_addition,      ONLY : compute_dipole_integrals
  !
  IMPLICIT NONE
  !
  ! global variables
  ...
  COMPLEX(DP), ALLOCATABLE  :: psi_r_3d(:,:,:)
  LOGICAL,     INTENT(IN)    :: metalcalc
  !
  ! local variables
